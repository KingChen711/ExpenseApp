  ...
  return (
    <div className="bg-black rounded-md md:p-8 p-3 mt-14 w-[500px] md:w-[800px] max-w-[90%] mx-auto mb-8">
      ...
      {expensesFilterByYear()?.map((expense) => {
        return <ExpenseItem setIsEditing={props.setIsEditing} key={expense._id} data={expense} />
      })}
    </div>
  )
}
...
function ExpenseItem(props) {
  ...
  function handleDeleteExpense() {
    setExpenses((prev) => {
      return prev.filter(expense => expense._id !== _id);
    })
    axios.delete(`${URL_BASE}/api/expenses/${_id}`);
  }
